...
class Evaluator():
    ...
    def evaluate(self, df_origin, measure, num_rec, mode = 'ASIS', cap_prop=None):
        ...
        if measure == 'precision':
            ...
            return float(np.sqrt(np.nanmean(np.square(df.loc[:, self.colname_outcome] - df.loc[:, self.colname_prediction]))))
        elif measure == 'logloss':
            return float(-np.nanmean(df.loc[:, self.colname_outcome] * np.log(df.loc[:, self.colname_prediction]) +
                                    (1-df.loc[:, self.colname_outcome]) * np.log(1-df.loc[:, self.colname_prediction])))
        else:
