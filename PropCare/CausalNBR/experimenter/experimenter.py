...
from recommender import RandomBase, PopularBase, NeighborBase, CausalNeighborBase, LMF, ULMF, DLMF, CausE, CausEProd, MF
...
class Experimenter():
    def __init__(self, colname_user='idx_user', colname_item='idx_item',
                 ...
                 monitor=['AUC', 'CPrec_10', 'CPrec_100', 'CDCG_100000']):
        ...
        self.evaluator = Evaluator(colname_outcome=self.colname_outcome)
        self.default_params = {
            # model condition
            'recommender': 'DLMF',
            'dim_factor': 200,
    ...
    def validate(self, recommender, df_train, df_vali, num_loop, interval_eval, eval_metrics, eval_train_data=False, check_factors=False):
        ...
        for n in np.arange(num_loop):
            ...
            for eval_metric in eval_metrics.split('-'):
                ...
                if 'AUC' in eval_metric or 'AR' in eval_metric or 'RMSE' in eval_metric or 'logloss' in eval_metric:
                    measure = eval_metric
                    num_rec = np.nan
                else:
    ...
    def do_params(self, params, df_train, df_vali, num_users=-1, num_items=-1, save_result_file=None):
        ...
        elif params['recommender'] == 'MF':
            recommender = MF(num_users=num_users, num_items=num_items,
                              colname_user=self.colname_user, colname_item=self.colname_item,
                              colname_outcome=self.colname_outcome, colname_prediction=self.colname_prediction,
    ...
    def try_params(self, list_params, df_train, df_vali, num_users=-1, num_items=-1, save_result_file=None):
        ...
        if num_users < 0:
            ...
            num_users_vali = np.max(df_vali.loc[:, self.colname_user].values) + 1
            if num_users_vali > num_users:
                num_users = num_users_vali
        if num_items < 0:
            num_items = np.max(df_train.loc[:, self.colname_item].values) + 1
            num_items_vali = np.max(df_vali.loc[:, self.colname_item].values) + 1
        ...
        for params in list_params:
            ...
            if 'df_all' in locals():
                df_all = pd.concat([df_all, df_result]) #df_all.append(df_result)
            else:
                df_all = df_result
