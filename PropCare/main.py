    ...
    for th in [0.6, 0.7]:
        for phi in [0.1, 0.2, 0.3]:
            ...
            for epoch in range(flag.repeat):
                ...
                rel_pred = np.where(r_pred_t >= th, 1.0, 0.0)
                ...
                if flag.rec_train:
                    recommender.train(train_df, plotpath + flag.add, phi, iter=itr)
                ...
                if flag.dataset == 'd' or 'p':
                    for t in range(num_times):
                        ...
                        test_df_t["outcome_estimate"] = np.where(outcome_estimate >= th, 1.0, 0.0)
            ...
            with open(plotpath + "/result_" + flag.dataset +".txt", "a+") as f:
                print("Threshold: ", th, ", Phi: ", phi, file=f)
